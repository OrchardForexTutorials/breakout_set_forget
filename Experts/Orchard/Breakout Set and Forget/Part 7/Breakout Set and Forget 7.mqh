...
// Using a structure to hold entry/tp levels
struct STradeData
{
...
void UpdateBreakEven() {
   ...
   int        groupCount = LoadGroupPrice( groupList );

   // Now loop through all trades and check if they have reached be
   // 	be if ticket1 does not exist
   for ( int i = groupCount - 1; i >= 0; i-- ) {

      if ( groupList[i].trades[0].ticket > 0 ) continue; // Ticket 1 still open
...
int LoadGroupPrice( SGroupData &groupList[] ) {
   ...
   for ( int i = PositionsTotal() - 1; i >= 0; i-- ) {
      ...
      // Now find an existing group
      int      index    = -1;
      datetime timeOpen = PositionTimeOpen();
      for ( int j = 0; j < groupCount; j++ ) {
         if ( MathAbs( timeOpen - groupList[j].time ) < 300 ) { // found a group
            index = j;
            break;
         }
      }
      ...
      // Now fill in based on the magic
...
#ifdef __MQL4__
   bool success = OrderModify( ( int )trade.ticket, trade.priceOpen, trade.priceOpen, trade.takeProfit, 0 );
#endif
#ifdef __MQL5__
